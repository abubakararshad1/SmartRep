...
*** Test Cases ***
Verify the Login functionality with an invalid username and password.
    [Documentation]    Verify the Login functionality with an invalid username and password.
    [Tags]      Smoke   Critical
...
Verify the Login functionality with an valid username and password.
    [Documentation]    Verify the Login functionality with an valid username and password.
    [Tags]      Smoke   Critical
#   Login to Salesforce Flow
    Sleep    5s
    Wait Until Element Is Visible    xpath=//android.widget.Button[@text="Log In to Sandbox"]     20s
...
Verify that the user can Sync data
    [Documentation]   Verify that the user can Sync data.
    [Tags]      Smoke   Critical
#     click sync button
    Sleep    10s
